...
from icv_src.icv_datasets.vqa_dataset import VQADataset
from icv_src.icv_datasets.caption_dataset import CaptionDataset

class VQAICVRLDataModule(pl.LightningDataModule):
    def __init__(self, data_cfg, prompt_manager: LMMPromptManager, prompt_processor: LMMPromptProcessor):
        super().__init__()
        self.prompt_processor = prompt_processor
        self.prompt_processor.tokenizer.padding_side = "right"
        self.prompt_manager = prompt_manager
        self.collator_data = partial(collator_rl_data, prompt_processor=self.prompt_processor)
        ...
        if stage == "fit":
            if self.data_cfg.task.task_name == "vqa":
                self.train_ds = VQADataset(
                    ...
                    split="train",
                    val_ann_file=getattr(self.data_cfg.task.datasets, "val_ann_file", None),
...
def collator_rl_data(data_list, prompt_processor):
    sample = data_list[0]
    ...
    query_input = prompt_processor.prepare_input(
        batch_prompts=query_prompt, padding=True, truncation=True, add_eos_token=True
    )
    ...
    input_prompts = [ice + query for ice, query in zip(ice_prompt, query_prompt)]

    inputs = prompt_processor.prepare_input(
        input_prompts, return_tensors="pt", padding=True, truncation=True, add_eos_token=True
    )
    in_context_length = (
        (
            ice_input[prompt_processor.input_ids_field] != prompt_processor.tokenizer.pad_token_id
        )
    ).sum(dim=1)
    query_x_without_bos_length = (
        (
            query_x[prompt_processor.input_ids_field] != prompt_processor.tokenizer.pad_token_id
        )
        & (
            query_x[prompt_processor.input_ids_field] != prompt_processor.tokenizer.bos_token_id
        )
    ).sum(dim=1)
    ...
    # Extract answer from the query_prompt
    query_x_text = data_dict["query_x"]
    answer_text = query_prompt[0][1].split("Short answer:")[1]


    """
    query_prompt: question + answer ( no icl)
    ice_prompt: in-context examples ( no query)
    query_x: only question
    input_prompts: in-context examples + query (with answer)
    """
    ...
    return {
        ...
        "query_x_input": query_x,
        "answer_text": answer_text,
        "query_x_text": query_x_text,
    }
